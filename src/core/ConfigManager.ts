...
export class ConfigManager {
  ...
  getTokenClassName(keyword: string): string | null {
    ...
    return type
      ? `rcse-${this._camelToKebab(type.className)}`
      : null;
  }
   ...
   */
  getKeywordGroups(): Record<
    string,
    { keywords: string[]; style: { color: string; fontWeight: string | number } }
  > {
    const groups: Record<
      string,
      { keywords: string[]; style: { color: string; fontWeight: string | number } }
  ...
  static validateConfig(config: SyntaxConfig): boolean {
    ...
    for (const type of config.types) {
      if (!type.className || !type.highlightColor || !type.parentType) {
        throw new Error(
          'Each type must have className, highlightColor, and parentType'
        );
      }
    }
